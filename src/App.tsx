...
const BACKEND_URL: string = env.REACT_APP_BACKEND_URL;
const DISTRICTS_ENDPOINT: string = env.REACT_APP_DISTRICTS_ENDPOINT;
const AREAS_ENDPOINT: string = env.REACT_APP_AREAS_ENDPOINT;
...
const svgToDataURL = (svg: React.FunctionComponent<React.SVGProps<SVGSVGElement>>): string => {
  const svgElement = new DOMParser().parseFromString(
    ReactDOMServer.renderToStaticMarkup(<Logo />),
    'image/svg+xml'
  ).documentElement;
...
const useMeta = () => {
  useMetaTag(
    'description',
    'Green Spaces Strategy Development — проєкт спрямований на збереження та розвиток зелених зон в місті, розбудову сталого навколишнього середовища та підвищення якості життя мешканців міст.'
  );
  useMetaTag(
    'keywords',
    'Green Spaces Strategy Development, Dnipro, зелені зони, зелені території, зелені насадження, зелені насадження міста Дніпро, зелені зони міста Дніпро, зелені території міста Дніпро, зелені насадження Дніпро, зелені зони Дніпро, зелені території Дніпро, парки Дніпра, стратегія розвитку зелених зон'
  );
};
      ...
      <Routes>
        <Route path="/" element={<MainLayout />}>
          {cityData && (
            <Route
              index
              element={<HomePage greenAreas={cityData.greenAreas} districts={cityData.districts} />}
            />
          )}
